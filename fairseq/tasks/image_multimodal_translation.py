...
def load_langpair_dataset(
    ...
    for image_feat_path in image_feat_path_list:
        ...
        print(f"feat_pth_path: {feat_pth_path}")
        assert os.path.exists(feat_pth_path) == True, 'not found image feature'
        img_dataset = ImageDataset(feat_pth_path, mask_pth_path, shuffle_img=args.get("shuffle_img", False) and split=="train")
...
class ImageMMTTask(LegacyFairseqTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--target-prompt', default=0.0, type=float,
                    help='sample target word as prompt')
        parser.add_argument('--dynamic-mask-color', default=0.8, type=float,
                    help='dynamic mask colors in ext data')
        parser.add_argument('--shuffle-img', action="store_true",
                    help='shuffle images')
        parser.add_argument('--shuffle-prompt', action="store_true",
                    help='shuffle prompts')
        parser.add_argument('--image-mask', default=0.0, type=float,help='mask images for triples in training')
        parser.add_argument('--prompt-mask', default=0.0, type=float,help='mask prompts for triples in training')
        parser.add_argument('--vis-fusion', action="store_true", help='visualization of fusion')
