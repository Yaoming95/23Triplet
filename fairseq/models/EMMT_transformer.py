...
class TransformerEncoder(FairseqEncoder):
    ...
    def fuse_ratio(self, text, idx, image, image_mask, text_mask):
        ...
        return torch.norm(gate * output) / torch.norm(text)
    ...
    def get_fuse_ratio(
        ...
        if not self.is_fusion_top:
            for img, img_mask in zip(imgs_list, img_masks_list):
                img = img.transpose(0, 1)
                return self.fuse_ratio(x, idx, img, img_mask, text_mask=src_tokens.ne(self.padding_idx))
